import get from '@minimessage-js/fetch-translations';
...
class DecorationMap {
  ...
  get(decoration) {
    return TextDecoration.State.fromOrdinal(this._value >> TextDecoration.ordinal(decoration) * 2 & 3);
  }
  ...
  ((ShowItem2) => {
    function create2(item, count) {
      ...
      return Object.freeze({
        item: defineAccessor(
          () => item,
          (item2) => create2(Key.key(item2), count)
        ),
    ...
    function showItem2(item, count) {
      return create2(Key.key(item), count);
    }
...
((ComponentCompaction2) => {
  ...
  function joinText(a, b) {
    return Component.text(a.content() + b.content()).style(a.style()).append(...b.children());
  }
...
class AbstractScopedComponent {
  constructor(extra, children = [], style = Style.empty()) {
...
((SpriteObjectContents2) => {
  SpriteObjectContents2.DEFAULT_ATLAS = Key.key("minecraft:blocks");
...
((ObjectContents2) => {
  function spriteConstructor() {
    ...
    if (arguments.length === 1) {
      sprite2 = Key.key(arguments[0]);
    } else if (arguments.length === 2) {
      atlas = Key.key(arguments[0]);
      sprite2 = Key.key(arguments[1]);
    } else {
...
const truncateColor = (() => {
  const NAMED = Object.values(NamedTextColor.NAMES);
  const distance = ((a, b) => {
    let dist = 0;
    const sum = ((component) => {
  ...
  return ((color) => {
    if (NamedTextColor.isNamed(color)) return color;
    let closest = NAMED[0];
...
const _LegacyColorComponentRenderer = class _LegacyColorComponentRenderer extends AbstractComponentRenderer {
  ...
  _renderComponent(component) {
    ...
    if (shadowColor !== null) {
      ...
      let textColor = TextColor.color(value & 16777215);
...
((ComponentFlattener2) => {
  ...
  const BASIC = builder().mapper(KeybindComponent.TYPE, (c) => c.keybind()).mapper(SelectorComponent.TYPE, (c) => c.pattern()).mapper(TextComponent.TYPE, (c) => c.content()).mapper(TranslatableComponent.TYPE, (c) => {
    ...
    if (contents.type === "sprite") {
      ...
      return Key.equals(atlas, SpriteObjectContents.DEFAULT_ATLAS) ? `[${sprite.asMinimalString()}]` : `[${sprite.asMinimalString()}@${atlas.asMinimalString()}]`;
...
__publicField$w(PlainTextComponentSerializerImpl, "DEFAULT_FLATTENER", ComponentFlattener.basic().toBuilder().unknownMapper((component) => {
  throw new Error(`Don't know how to turn component of type "${component.type}" into a string`);
}).build());
var PlainTextComponentSerializer;
...
})(PlainTextComponentSerializer || (PlainTextComponentSerializer = {}));
...
const _JsonComponentSerializerImpl = class _JsonComponentSerializerImpl {
  ...
  deserialize(input) {
    ...
    for (const key of Object.keys(input)) {
      ...
      switch (qual) {
        ...
        case "color":
          ...
          let color = TextColor.fromHexString(jsonColor);
          if (color === null) {
    ...
    if (text !== null) {
      ...
      if (nbtBlock !== null) {
        ...
        ret = Component.entityNBT(nbt, nbtEntity).interpret(nbtInterpret).separator(separator);
      } else if (nbtStorage !== null) {
        ret = Component.storageNBT(nbt, nbtStorage).interpret(nbtInterpret).separator(separator);
      } else {
  ...
  deserializeClickEvent(event) {
    ...
    switch (action) {
      case "open_url":
        return ClickEvent.openUrl(event.url);
      case "open_file":
        return ClickEvent.openFile(event.path);
      case "run_command":
        return ClickEvent.runCommand(event.command);
      case "suggest_command":
        return ClickEvent.suggestCommand(event.command);
      case "change_page":
        return ClickEvent.changePage(event.page);
      case "copy_to_clipboard":
        return ClickEvent.copyToClipboard(event.value);
      case "show_dialog":
...
__publicField$t(_JsonComponentSerializerImpl, "CLICK_EVENT_SERIALIZER", (() => {
  const handlers = new ClickEvent.Handlers();
...
__publicField$t(_JsonComponentSerializerImpl, "HOVER_EVENT_SERIALIZER", (() => {
  const handlers = new HoverEvent.Handlers();
...
})(JsonComponentSerializer || (JsonComponentSerializer = {}));
...
class MiscDomEffectImpl {
  ...
  deserialize(value) {
    const parsed = JSON.parse(value);
    return JsonComponentSerializer.json().deserialize(parsed);
  }
}
var MiscDomEffect;
((MiscDomEffect2) => {
...
((Chunks2) => {
  ...
  function parse(text) {
    ...
    while (hasNext()) {
      ...
      literal(true);
      if (!hasNext()) {
        throw new Error(`Dangling escape sequence at end of lang string "${text}"`);
      }
      c = next();
      ...
      throw new Error(`Illegal escape sequence at index ${head} of lang string "${text}"`);
    }
    literal(false);
    return ret;
  }
...
const _HtmlComponentRenderer = class _HtmlComponentRenderer extends AbstractComponentRenderer {
  ...
  _open(component, writer) {
    ...
    if (underlined !== TextDecoration.State.NOT_SET || strikethrough !== TextDecoration.State.NOT_SET) {
...
__publicField$l(_HtmlComponentRenderer, "HOVER_EVENT_RENDERER", (() => {
  ...
  handlers.register(HoverEvent.Action.SHOW_ENTITY, (event, context) => {
    ...
    const text = name !== null ? PlainTextComponentSerializer.plainText().serialize(name) : event.value().type();
    context.property("title", text);
  });
...
let HtmlComponentRenderer = _HtmlComponentRenderer;
((HtmlComponentRenderer2) => {
  const INSTANCE = new HtmlComponentRenderer2(Translations.empty());
  function renderer(translations = Translations.empty()) {
...
})(HtmlComponentRenderer || (HtmlComponentRenderer = {}));
...
class MiniMessageParser {
  ...
  _processTokens(sb, richMessage, context, tagHandler) {
    const combinedResolver = TagResolver.builder().resolver(this.tagResolver).resolver(context.extraTags()).build();
...
const _ColorTagResolver = class _ColorTagResolver {
  ...
  //
  has(name) {
    return _ColorTagResolver.isColorOrAbbreviation(name) || name in NamedTextColor.NAMES || name in _ColorTagResolver.COLOR_ALIASES || TextColor.fromHexString(name) !== null;
  }
  resolve(name, args, ctx) {
    if (!this.has(name))
      return null;
    ...
    if (current.type === "text") {
      if (current.content().length !== 0) {
        ...
        while (head < content.length) {
          ...
          const child = Component.text(String.fromCodePoint(codePoint)).style(current.style().color(this.color()));
          this.advanceColor();
          parent = parent.append(child);
        }
        return parent;
      } else {
        return Component.empty().style(current.style());
      }
    } else {
...
__publicField$4(_AbstractColorChangingTag, "LENGTH_CALCULATOR", ComponentFlattener.builder().mapper(TextComponent.TYPE, (c) => c.content()).unknownMapper(() => "_").build());
...
__publicField$3(_GradientTagImpl, "DEFAULT_WHITE", TextColor.color(16777215));
__publicField$3(_GradientTagImpl, "DEFAULT_BLACK", TextColor.color(0));
let GradientTagImpl = _GradientTagImpl;
var GradientTag;
((GradientTag2) => {
  GradientTag2.GRADIENT = "gradient";
  GradientTag2.RESOLVER = TagResolver.dynamic(GradientTag2.GRADIENT, GradientTagImpl.create);
...
((DecorationTag2) => {
  ...
  function resolvers(decoration, ...aliases) {
    ...
    for (const name of names) {
      resolvers2[head++] = TagResolver.dynamic(name, (args) => create(decoration, args));
      resolvers2[head++] = TagResolver.resolver(`${DecorationTag2.REVERT}${name}`, createNegated(decoration));
    }
  ...
  DecorationTag2.RESOLVERS = ((stream) => {
    ...
    for (const resolvers2 of stream) {
      ret[resolvers2.decoration] = TagResolver.builder().resolvers(...resolvers2.resolvers).build();
    }
    return Object.freeze(ret);
  })([
  ...
  DecorationTag2.RESOLVER = TagResolver.builder().resolvers(...Object.values(DecorationTag2.RESOLVERS)).build();
...
((HoverTag2) => {
  ...
  HoverTag2.RESOLVER = TagResolver.dynamic(HoverTag2.HOVER, create);
  let ActionHandler;
  ((ActionHandler2) => {
...
((ClickTag2) => {
  ...
  ClickTag2.RESOLVER = TagResolver.dynamic(ClickTag2.CLICK, create);
})(ClickTag || (ClickTag = {}));
...
((KeybindTag2) => {
  ...
  KeybindTag2.RESOLVER = TagResolver.dynamic(KeybindTag2.KEYBIND, create);
  ...
  function create(args, ctx) {
    ...
    if (!args.hasNext()) {
      ...
      if (outerLayer !== TriState.NOT_SET) {
        return Tag.selfClosingInserting(Component.object(
          ObjectContents.playerHead().hat(outerLayer === TriState.TRUE).build()
        ));
      }
    } else {
    ...
    if (UUIDv4_PATTERN.test(argument)) {
      return Tag.selfClosingInserting(Component.object(
        ObjectContents.playerHead().id(argument).hat(TriState.resolve(outerLayer, true)).build()
      ));
    }
    if (argument.indexOf("/") !== -1) {
      return Tag.selfClosingInserting(Component.object(
        ObjectContents.playerHead().texture(argument).hat(TriState.resolve(outerLayer, true)).build()
      ));
    }
    return Tag.selfClosingInserting(Component.object(
      ObjectContents.playerHead().name(argument).hat(TriState.resolve(outerLayer, true)).build()
    ));
  }
  SequentialHeadTag2.HEAD = "head";
  SequentialHeadTag2.RESOLVER = TagResolver.dynamic(SequentialHeadTag2.HEAD, create);
...
((TranslatableFallbackTag2) => {
  ...
  TranslatableFallbackTag2.RESOLVER = TagResolver.dynamic(
...
((TranslatableTag2) => {
  ...
  TranslatableTag2.RESOLVER = TagResolver.dynamic(
...
((InsertionTag2) => {
  ...
  InsertionTag2.RESOLVER = TagResolver.dynamic(InsertionTag2.INSERTION, create);
...
((FontTag2) => {
  ...
  FontTag2.FONT = "font";
  FontTag2.RESOLVER = TagResolver.dynamic(FontTag2.FONT, create);
})(FontTag || (FontTag = {}));
...
((RainbowTag2) => {
  ...
  RainbowTag2.RESOLVER = TagResolver.dynamic(RainbowTag2.RAINBOW, create);
...
((TransitionTag2) => {
  ...
  TransitionTag2.RESOLVER = TagResolver.dynamic(TransitionTag2.TRANSITION, create);
})(TransitionTag || (TransitionTag = {}));

var ResetTag;
((ResetTag2) => {
  ResetTag2.RESET = "reset";
  ResetTag2.RESOLVER = TagResolver.resolver(ResetTag2.RESET, ParserDirectiveTag.RESET);
})(ResetTag || (ResetTag = {}));
...
((NewlineTag2) => {
  ...
  NewlineTag2.BR = "br";
  NewlineTag2.RESOLVER = TagResolver.dynamic(NewlineTag2.NEWLINE, create, NewlineTag2.BR);
...
((SelectorTag2) => {
  ...
  SelectorTag2.RESOLVER = TagResolver.dynamic(SelectorTag2.SELECTOR, create, SelectorTag2.SEL);
...
((ScoreTag2) => {
  ...
  ScoreTag2.RESOLVER = TagResolver.dynamic(ScoreTag2.SCORE, create);
})(ScoreTag || (ScoreTag = {}));
...
((NbtTag2) => {
  ...
  NbtTag2.NBT = "nbt";
  NbtTag2.DATA = "data";
  NbtTag2.RESOLVER = TagResolver.dynamic(NbtTag2.NBT, resolve, NbtTag2.DATA);
})(NbtTag || (NbtTag = {}));
...
((PrideTag2) => {
  ...
  PrideTag2.RESOLVER = TagResolver.dynamic(PrideTag2.PRIDE, create);
})(PrideTag || (PrideTag = {}));
...
((ShadowColorTag2) => {
  ...
  const PRIMARY_RESOLVER = TagResolver.dynamic(ShadowColorTag2.SHADOW_COLOR, create);
  const NONE_RESOLVER = TagResolver.resolver(
    SHADOW_NONE,
    Tag.styling((s) => s.shadowColor(ShadowColor.none()))
  );
  ShadowColorTag2.RESOLVER = TagResolver.builder().resolvers(PRIMARY_RESOLVER, NONE_RESOLVER).build();
...
((SpriteTag2) => {
  ...
  SpriteTag2.RESOLVER = TagResolver.dynamic(SpriteTag2.SPRITE, create);
})(SpriteTag || (SpriteTag = {}));

var StandardTags;
((StandardTags2) => {
  const ALL = TagResolver.builder().resolvers(
...
class MiniMessageImpl {
  ...
  toHTML(component, target, elementFactory) {
    ...
    if (target) {
      const writer2 = HtmlWriter.dom(target, elementFactory);
  ...
  _newContext(input, resolvers) {
    ...
    if (resolvers.length === 0) {
      ...
      extraTags = TagResolver.builder().resolvers(...resolvers).build();
    }
...
class MiniMessageBuilderImpl {
  constructor(serializer) {
    __publicField(this, "_tagResolver", StandardTags.defaults());
    ...
    __publicField(this, "_translations", Translations.empty());
    if (serializer) {
...
if (typeof window !== "undefined") {
  window.adventure.fetchTranslations = get;
  window.adventure.minecraftTranslations = get;
}

export { AbstractComponentRenderer, ClickEvent, Component, ComponentFlattener, FlattenerListener, HoverEvent, HtmlComponentRenderer, HtmlWriter, JsonComponentSerializer, Key, LegacyColorComponentRenderer, MiniMessage, NamedTextColor, ObjectContents, PlainTextComponentSerializer, ShadowColor, StandardTags, Style, Tag, TagResolver, TextColor, TextDecoration, Translations };
